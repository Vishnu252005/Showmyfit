...
export const AISearchOptimization = {
  ...
  generateSemanticKeywords: (primaryKeyword: string) => {
    const keywordMap: { [key: string]: string[] } = {
      'local marketplace': ['community shopping', 'neighborhood stores', 'local business', 'regional commerce', 'nearby shopping'],
  ...
  generateFeaturedSnippetContent: (topic: string) => {
    const snippetTemplates = {
      'what is': `${topic} is a local marketplace platform that connects customers with nearby stores, offering a convenient way to shop from local businesses online.`,
